<x-app-layout>
    <div class="min-h-screen bg-gray-900 py-10 px-4">
        <div class="max-w-xl mx-auto bg-gray-800/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <h1 class="text-2xl font-bold text-white mb-6">Edit Fasilitas</h1>
            ...
            <form method="POST" action="{{ route('admin.facilities.update', $facility->id) }}" class="space-y-5">
                    ...
                    <select name="studio_id" required
                        ...
                        @foreach($studios as $studio)
                            <option value="{{ $studio->id }}" class="text-gray-800"
                                {{ $facility->studio_id == $studio->id ? 'selected' : '' }}>
                                {{ $studio->nama_studio }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nama_fasilitas" value="{{ old('nama_fasilitas', $facility->nama_fasilitas) }}" required
                ...
                <div>
                    ...
                    <select name="kategori" required
                            class="w-full rounded-xl bg-white/20 border border-white/30 text-white backdrop-blur-sm px-4 py-3 focus:ring-2 focus:ring-violet-500 focus:outline-none">
                        <option value="Alat Musik" class="text-gray-800" {{ $facility->kategori == 'Alat Musik' ? 'selected' : '' }}>Alat Musik</option>
                        <option value="Furniture" class="text-gray-800" {{ $facility->kategori == 'Furniture' ? 'selected' : '' }}>Furniture</option>
                        <option value="Elektronik" class="text-gray-800" {{ $facility->kategori == 'Elektronik' ? 'selected' : '' }}>Elektronik</option>
                        <option value="Ruangan" class="text-gray-800" {{ $facility->kategori == 'Ruangan' ? 'selected' : '' }}>Ruangan</option>
                        <option value="Internet" class="text-gray-800" {{ $facility->kategori == 'Internet' ? 'selected' : '' }}>Internet</option>
                        <option value="Lainnya" class="text-gray-800" {{ $facility->kategori == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
